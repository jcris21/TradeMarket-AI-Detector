# Spec: enrichment-type-field

## Purpose

The `enrichment-type-field` capability adds an `enrichment_type` column to `analysis_results` and the corresponding field to the `AssetAnalysis` model, so downstream consumers (API responses, performance segmentation) can distinguish between enrichment sources (`trader_chart`, `auto_screenshot`, or unenriched `null`).
