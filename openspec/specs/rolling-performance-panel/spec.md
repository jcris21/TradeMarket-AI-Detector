# Spec: rolling-performance-panel

## Purpose

The `rolling-performance-panel` capability renders a `PerformanceSummaryPanel` frontend component that displays aggregated trading performance metrics — hit ratio, profit factor, and realized R/R — gated behind a 30-signal calibration phase. The panel is always visible inside `OpportunitiesPanel` and refreshes automatically after each analysis run.
